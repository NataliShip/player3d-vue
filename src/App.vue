<template>
  <div class="player3d">
    <div>
      <h2 class="player3d__header">
        3D обзор товара, через покадровую смену фотографий
      </h2>
      <p class="player3d__description">
        Клик на превью для старта. Повторный клик остановит просмотр. Можно
        поворачивать с помощью мыши
      </p>
      <div class="player3d__columns">
        ...
        <div
          id="start"
          :class="this.playerActive ? 'hide' : 'preview'"
          @click="this.playerStart"
        >
          <img
            v-if="this.images.length > 0"
...
export default {
  ...
  created() {
    axios
      .get("https://private-5adf60-images3dapi.apiary-mock.com/images")
      .then(response => {
        // handle success
